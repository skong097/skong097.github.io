---
title: "Kevin 3D Patrol Simulator — 개발 문서 (2025-02-16 / Phase 3 v3.2 완료)"
date: 2026-03-21
draft: true
tags: ["robotics", "slam", "patrol"]
categories: ["robotics"]
description: "**프로젝트**: Kevin 3D Patrol Simulator **파일**: `kevin_3d_sim.py` (2,653 lines) **플랫폼**: Pygame + PyOpenGL"
---

# Kevin 3D Patrol Simulator — 개발 문서 (2025-02-16 / Phase 3 v3.2 완료)

**프로젝트**: Kevin 3D Patrol Simulator  
**파일**: `kevin_3d_sim.py` (2,653 lines)  
**플랫폼**: Pygame + PyOpenGL  
**Python**: 3.10+  
...
| 2025-02-15 | v1.3 | A* 경로 탐색 시각화 (탐색 과정 애니메이션), 순찰 경로/장애물 충돌 검증 재배치, LiDAR 기반 장애물 회피 순찰 |
| 2025-02-15 | v1.4 | Costmap 시각화 (Global inflation + Local LiDAR), 절차적 사운드 시스템 (경보·모터·차임) |
| 2025-02-15 | v1.5 | 순찰 경로 편집기 (WP 추가/삭제/드래그 이동, 충돌 검증, 경로 연결선) — Phase 1 완료 |
| 2025-02-15 | v2.0 | DataProvider 추상화 레이어 — ABC 인터페이스 + SimDataProvider + ROS2DataProvider 스텁 (Phase 2) |
| 2025-02-15 | v3.0 | 통합 모니터링 GUI Dashboard — SLAM Map, Sensor Plot, Topic Monitor, Robot Status (Phase 3) |
| 2025-02-16 | v3.1 | Dashboard 레이아웃 개선 — 2×2 그리드 + 우측 사이드바, Camera Feed, 알림 시스템 (Phase 3) |
| 2025-02-16 | v3.2 | Phase 3 완료 — Nav2 Goal 클릭, Sensor Plot IMU/Motor, 모드 전환, Command Bar 동작 연결 |
...
│
├── AStarPlanner          — A* 경로 탐색 + 탐색 과정 애니메이션 (v1.3)
│     plan()                — A* 실행 + 애니메이션 데이터 기록
│     update_animation()    — 프레임별 탐색 과정 재생
│     draw_3d()             — open/closed/path 3D 시각화
│     draw_on_minimap()     — 미니맵 위 경로 오버레이
│
├── CostmapVisualizer     — Global/Local Costmap 시각화 (v1.4)
│     _build_global()       — 정적 장애물 + inflation 레이어 빌드
│     update_local()        — LiDAR 결과로 local costmap 실시간 업데이트
│     draw_3d_global()      — global costmap 바닥 히트맵 (DisplayList)
│     draw_3d_local()       — local costmap 실시간 히트맵
│     draw_on_minimap()     — 미니맵 위 costmap 오버레이
│
├── SoundSystem           — 절차적 사운드 생성 (v1.4)
│     _generate_sounds()    — numpy 파형 → pygame Sound (6종)
│     play() / stop()       — 재생·정지 제어
...
#### 3-5. v3.1 업데이트 — 레이아웃 개선 + Camera Feed + 알림 시스템 (2025-02-16)

**레이아웃 재구성**: 기존 3분할(SLAM | Sensor | Topic+Status) → 설계 문서 원안 2×2 그리드 + 우측 사이드바
...
| 위젯 | 클래스 | 기능 |
|---|---|---|
| **Camera Feed** | `CameraFeedWidget` | SIM: LiDAR 기반 합성 1인칭 뷰 (레이캐스팅 의사 3D, 스캔라인 효과). LIVE: /image_raw numpy→QImage. 감지 바운딩 박스 오버레이 (코너 마커, 거리 표시). HUD (프레임 카운터, REC 깜박임, 크로스헤어, FOV 표시) |
| **Alert Toast** | `AlertToast` | 4종 타입별 색상 (danger/warning/info/success), 5초 자동 소멸, 최대 3개 스택, 닫기 버튼 |
...
#### 3-6. v3.2 업데이트 — Phase 3 완료 (2025-02-16)

**1. SLAM Map 클릭 → Nav2 Goal 전송**

| 기능 | 설명 |
|---|---|
| SET_GOAL 모드 | Command Bar 버튼 토글 → 맵에 빨간 점선 테두리 + "🎯 SET GOAL MODE" 표시, 커서 십자 변경 |
| 좌클릭 | Goal 설정 → 빨간 펄스 타겟 마커 (외부 링 펄스 + 내부 점 + 십자선) + 좌표 라벨 + 로봇→Goal 점선 |
| 우클릭 | Goal 취소 |
| 도달 판정 | 로봇-Goal 거리 1.0m 이내 시 녹색 체크 마커로 변환 |
| 좌표 변환 | `_screen_to_world()` — 화면 픽셀 → 월드 좌표 역변환, 맵 범위 검증 |
| 시그널 | `goal_set(float, float)` — DashboardWindow에서 알림 + Phase 4 Nav2 연동 준비 |

**2. Sensor Plot 확장 — 2단 서브플롯**

```
┌─────────────────────────────────────┐
│  상단: LiDAR min (녹) + Speed×5 (청) │
├─────────────────────────────────────┤
│  하단: IMU ang.vel (보라) + Motor (주황) │
│        X축 연동 (setXLink)            │
└─────────────────────────────────────┘
```

- IMU angular velocity: `/imu` 시뮬 (기본 0.3 rad/s + 사인파 + 가우시안 노이즈)
- Motor current: `/joint_states` 시뮬 (기본 0.8A + 부하 변동 + 노이즈)
- 정지 시 미세 노이즈만 표시 (대기 전류 0.05A)

**3. SIM/LIVE/REC 모드 전환**

| 모드 | 상태 표시 | Provider | 비고 |
|---|---|---|---|
| **SIM** | `● Connected` (녹색) | SimDataProvider | 현재 동작 |
| **LIVE** | `○ Waiting...` → `● LIVE` | ROS2DataProvider (Phase 4) | rosbridge ws://localhost:9090 |
| **REC** | `○ No bag file` → `▶ PLAYBACK` | BagDataProvider (Phase 4) | ros2 bag 재생 |

- 활성 모드 버튼 색상 강조 (SIM=파랑, LIVE=녹색, REC=주황)
- 전환 시 알림 토스트 + 상태바 메시지

**4. Command Bar 버튼 동작 연결**

| 버튼 | 동작 | 시각 피드백 |
|---|---|---|
| **▶ PATROL** | `start_patrol()` → 원형 순찰 + SLAM 점진 탐색 + WP 순환 | 녹색 채움 활성 |
| **⏸ STOP** | `stop_patrol()` → 위치 유지, speed=0, 대기 전류 | 빨간색 채움 활성 |
| **🔄 SLAM RESET** | `reset_slam()` → grid unknown 초기화 + Goal 클리어 | 알림 토스트 |
| **📍 SET GOAL** | Goal 모드 토글 → SLAM Map 클릭 대기 | 체크 토글 |
| **⚠ ALERTS** | 테스트 알림 랜덤 발생 | 토스트 팝업 |

**Phase 3 완료 상태**: 설계 문서 원안의 모든 대시보드 기능 구현 완료. Phase 4 (실전 배포) 진입 준비 완료.
...
├── core/                       ← 공유 핵심 모듈
│   ├── __init__.py
│   └── data_provider.py        ← DataProvider ABC + SimDataProvider + ROS2DataProvider (401 lines)
│
├── sim/                        ← 3D 시뮬레이터
│   ├── __init__.py
│   └── kevin_3d_sim.py         ← Pygame + OpenGL 시뮬레이션 (2,669 lines)
│
├── dashboard/                  ← 통합 모니터링 GUI
│   ├── __init__.py
│   └── app.py                  ← PyQt6 + pyqtgraph 대시보드 (1,366 lines, v3.2)
│
├── docs/                       ← 문서
│   ├── development.md          ← 개발 로드맵 (이 파일)
