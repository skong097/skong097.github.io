...
date: 2026-02-12
tags: ["pyqt6", "python", "ui", "theme", "dark-mode", "qss"]
categories: ["dev-tools"]
summary: "QSS 외부 파일 + 상수 딕셔너리 + paintEvent 동기화로 PyQt6 애플리케이션에 실시간 테마 전환을 구현한 방법"
cover:
  image: "images/theme-comparison.png"
  alt: "Cyber / Classic / JARVIS 3테마 비교"
draft: true
ShowToc: true
TocOpen: true
---
...
{{< figure src="/images/diagrams/pyqt6-theme-architecture.svg" alt="PyQt6의 QSS, paintEvent, PyQtGraph를 통합 관리하는 3계층 테마 시스템 아키텍처 구조도" >}}
...
```python
# 1. 메인 윈도우에서 테마 색상을 각 패널로 전파
def _apply_panel_bg_theme(self, theme_name: str):
    bg_color = PANEL_BG_THEMES.get(theme_name, (30, 30, 30))
    hl_color = PANEL_HIGHLIGHT_THEMES.get(theme_name, (255, 255, 255, 20))
    
    # 등록된 모든 커스텀 패널의 색상 변수 업데이트
    for panel in self.findChildren(CustomPanelWidget):
        panel.update_theme(bg_color, hl_color)

# 2. 커스텀 위젯 내에서의 paintEvent 활용
class CustomPanelWidget(QWidget):
    def update_theme(self, bg_tuple, hl_tuple):
        self._bg_color = QColor(*bg_tuple)
        self._hl_color = QColor(*hl_tuple)
        self.update()  # UI 갱신 (paintEvent 호출)
    ...
    def paintEvent(self, event):
        ...
        # 상단 1px 하이라이트 (입체감 부여)
        painter.setPen(QPen(self._hl_color, 1))
        painter.drawLine(15, 1, self.width() - 15, 1)
```
