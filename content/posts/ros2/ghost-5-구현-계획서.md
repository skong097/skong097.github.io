...
categories: ["ros2"]
description: "> GPS-denied Hazard Operation with Swarm Team — 5 Units > 작성일: 2026-03-15 | **v1.3 — 3가지 최종 보완 + 하드웨어 배치 다이어그램: 2026-03-"
---

# GHOST-5 구현 계획서

> GPS-denied Hazard Operation with Swarm Team — 5 Units  
> 작성일: 2026-03-15 | **v1.3 — 3가지 최종 보완 + 하드웨어 배치 다이어그램: 2026-03-15**  
> 기반 문서: GHOST5_research.md v2.1  
...
| v1.3 | 2026-03-15 | **3가지 최종 보완 + 센서 배치 다이어그램**: ① 섹션 9 — Redis Slave Lag 대응: add_victim() WAIT(1, 100ms) 동기 쓰기(sync_write=True 기본값), add_victim_async() 비동기 폴백, check_slave_lag_and_warn() 모니터링 메서드 추가 ② 섹션 10 — Temporal Consistency Filter: TEMPORAL_MIN_SEC(2.0s), TEMPORAL_DECAY_SEC(10.0s) 파라미터 추가; elevation_cells에 first_seen/last_seen 타임스탬프 기록; _publish_elevation()에서 Decay 삭제 + Duration Guard(지속 시간 미달 셀 미확정) 이중 필터 적용 ③ 섹션 8 — Election Storm 방지: BACKOFF_STEP_SEC(0.1s), _start_election_with_backoff()로 ID 기반 지연(Robot-5: 0s → Robot-1: 0.4s), _start_election_once() Backoff 중 VICTORY 수신 시 자동 취소 ④ 섹션 20 — Pinky Pro 센서 배치 최종 다이어그램 (LiDAR Z-stack+IMU 파이프라인, Comm Gradient Map 복귀 경로, Election+Redis HA 연동 흐름, 센서별 감지 범위 요약) | ① 섹션 8 — Split-Brain 방지 쿼럼(QUORUM_SIZE=3, /swarm/leader_dead 투표, MAX_ELECTION_RETRY 폴백) ② 섹션 10 — Ray-casting Clearing(Bresenham 선분, hits 카운터 감소, Ghost Trail 동적 제거, HITS_TO_OBSTACLE 확정 임계) ③ 섹션 11 — 로컬 미니마 탈출(Random Perturbation 2회 + Best-History Fallback 상위 3셀 평균) ④ 섹션 20 — XL330 슬립 감지 EKF 가중치 보정(SlipAwareEKFTuner, IMU-Encoder 속도 차이 임계값 기반 encoder_noise_cov 동적 전환) ⑤ 섹션 20 — RPLiDAR C1 Energy Saving Mode(LidarPowerManager, ACTIVE/STANDBY/HIBERNATE 3단계, 로봇 상태 + 통신 이벤트 연동) | ① 섹션 10 — BNO055 IMU Pitch/Roll 기반 동적 보정 (Ghost Obstacle 제거, R_imu 회전 행렬 + 폐기 임계값 설계) ② 섹션 9 — Redis HA 복제 (SLAVEOF + promote_replica_to_master, AOF 영속화, 복제 대상 분리 전략) ③ 섹션 11 — Communication Gradient Map (RSSI 지수 이동 평균 지도, gradient 방향 Safe Comm Zone 탐색, Foxglove 시각화) ④ 섹션 16 — Election Phase Write Lock (ElectionGuard, WriteBlockedError, X-Election-Status 헤더, 클라이언트 재시도 로직) |

---

*본 plan.md는 GHOST5_research.md v2.1의 모든 설계 결정을 구현 관점으로 재구성한 문서입니다.*  
*Phase 1 완료 후 실제 측정값으로 성능 목표를 갱신할 예정입니다.*
