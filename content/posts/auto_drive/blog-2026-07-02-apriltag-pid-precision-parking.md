# Nav2로 가까이 가고, AprilTag PID로 정확히 멈추기

로봇을 목적지까지 보내는 일은 Nav2가 잘한다. 지도 위에서 경로를 만들고, 장애물을 피하고, 목표 지점 근처까지 안정적으로 이동하는 데에는 Nav2가 적합하다.
...
precision node
  |
  +-- NavigateToPose -> Nav2 접근 주행
  |
  +-- AprilTag detection / TF -> tag 상대 pose 계산
  |
  +-- /cmd_vel -> 저속 PID/servo 정렬
```
...
```text
Nav2:
  지도 기준으로 목표물 근처까지 안전하게 이동

AprilTag PID:
  목표물 기준 상대 pose로 마지막 cm 단위 정렬
```
