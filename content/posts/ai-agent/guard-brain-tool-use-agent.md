...
{{< figure src="/images/diagrams/guard-brain-file-tree.svg" alt="guard_brain_agent 파일 구조 — config.py 설정값, tools.py 도구 함수 5개(Mock), prompts.py LLM 시스템 프롬프트, agent.py 에이전트 코어, main.py FastAPI 서버, requirements.txt" >}}
...
{{< figure src="/images/diagrams/guard-brain-tooluse-vs-react.svg" alt="현재 Tool Use는 상황에서 도구1·도구2·도구3을 거쳐 완료로 가는 선형 흐름이고, 다음 단계 ReAct는 도구 호출 사이에 재판단을 넣어 결과에 따라 계획을 바꾼다" >}}
