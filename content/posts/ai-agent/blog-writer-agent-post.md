...
{{< figure src="/images/diagrams/blog-writer-agent-flow.svg" alt="BlogWriterAgent.run() 흐름 — POST /agent/write 요청 → Step1 scan_references·Step2 read_file·Step3 classify_category(코드가 직접 실행)·Step4 LLM 본문 생성(LLM 전담)·Step5 write_post → Hugo 포스트 md 파일 저장" >}}
