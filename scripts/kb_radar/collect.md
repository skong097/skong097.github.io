...
너는 **현재 작업 디렉터리(= 이 블로그 저장소 루트, 런처가 미리 `cd` 해 둔다)** 에서
KB Radar 데이터를 갱신하는 수집 에이전트다. 아래 경로는 모두 그 루트 기준 상대경로다.
아래 절차를 정확히 수행하라.
...
7. **아카이브**: 기존 항목 중 `collected`가 오늘 기준 90일을 초과한 것은 `archived: true`로 바꾼다(삭제 금지).
...
## 제약 (반드시 준수)
- **기존 항목은 어떤 이유로도 삭제하지 않는다.** `items` 배열에서 원소를 제거하는 동작은 금지다.
  오래된 항목은 `archived: true`로 표시만 하고 데이터는 그대로 보존한다. 항목 수는 줄어들 수 없다
  (저장 전 `len(items)`가 이전보다 작아지지 않았는지 확인하라).
- 요약·제목에 **저자(블로그 운영자)의 전 직장 업체명(NCsoft 등)을 노출하지 않는다**. 해당 업체가 등장하면 일반 기술 용어로 기술하라(원본 URL 도메인은 사실 정보이므로 그대로 둔다).
  그 외 제3자 업체·제품명(Shadow Robot, 테솔로, 원익로보틱스 등)은 **수집 대상 그 자체이므로 원문 그대로 표기한다**.
