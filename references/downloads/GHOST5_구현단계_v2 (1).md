...
> 작성일: 2026-03-17 | **v1.2 — [B4] Inter-Robot 동적 장애물 등록 반영: 2026-03-17**  
...
> ⚠️ **[보완 B4] Inter-Robot 동적 장애물 등록** — 상세 내용은 [섹션 12.4](#124-b4--inter-robot-동적-장애물-등록-nav2-costmap-m06) 참조

**핵심 보완 요약**
- `inter_robot_costmap_layer.py`: `/robot_N/pose` 5대 수집 → TTL 0.3s → PointCloud2 → `/swarm/robot_poses_array` 5Hz 퍼블리시
- `nav2_params.yaml` `local_costmap`에 `robot_layer` (ObstacleLayer) 추가 — 이동 중 로봇 간 실시간 충돌 방지

**완료 조건**
- [ ] `NavigateToPose` Action으로 목적지 이동 성공
- [ ] 미끄러운 바닥에서 슬립 감지 로그 출력 확인
- [ ] EKF 오도메트리 정확도 < 5cm (1m 이동 기준)
- [ ] **[B4]** `/swarm/robot_poses_array` 5Hz 퍼블리시 확인
- [ ] **[B4]** 타 로봇 1m 이내 접근 시 Nav2 local costmap에 장애물 마킹 확인 (RViz2)
- [ ] **[B4]** 타 로봇 이동 후 0.3s 내 costmap 장애물 자동 삭제 확인
...
### 12.4 [B4] — Inter-Robot 동적 장애물 등록 (Nav2 Costmap) (M06)

**문제**

Nav2 local costmap은 LiDAR scan 기반 정적 장애물만 인식한다. 좁은 통로에서 두 로봇이 마주쳤을 때 서로를 빈 공간으로 인식하여 정면 충돌하거나 교착(Deadlock) 상태에 빠진다.  
MMPF `robot_repulsion`은 Frontier **선택 시점**에만 동작하므로, **이동 중** 발생하는 로봇 간 충돌은 별도로 대응해야 한다.

**보완 1 — InterRobotCostmapLayer 노드**
...
class InterRobotCostmapLayer(Node):
    """
    타 로봇 위치 → /swarm/robot_poses_array (PointCloud2) 퍼블리시.
    Nav2 local_costmap robot_layer(ObstacleLayer)가 동적 장애물로 반영.

    TTL 0.3s: 갱신 없으면 해당 로봇 장애물 자동 제거 (이동 후 잔상 방지)
    """
    PUBLISH_HZ   = 5.0
    POSE_TTL_SEC = 0.3
    ...
    def __init__(self, robot_id: int, total_robots: int = 5):
        ...
        for i in range(1, total_robots + 1):
            if i == robot_id:
                continue
            self.create_subscription(
                PoseWithCovarianceStamped, f'/robot_{i}/pose',
                lambda msg, rid=i: self._pose_cb(msg, rid), POSE_QOS
            )

        self._pub = self.create_publisher(
            PointCloud2, '/swarm/robot_poses_array', POSE_QOS
        )
        self.create_timer(1.0 / self.PUBLISH_HZ, self._publish)

    def _pose_cb(self, msg: PoseWithCovarianceStamped, rid: int):
        x = msg.pose.pose.position.x
        y = msg.pose.pose.position.y
        self._robot_positions[rid] = (x, y, time.monotonic())
    ...
    def _publish(self):
        ...
        for rid, (x, y, ts) in list(self._robot_positions.items()):
            if now - ts > self.POSE_TTL_SEC:
                del self._robot_positions[rid]  # TTL 초과 → 잔상 제거
                continue
            pts.append((x, y))
        ...
        data = bytearray()
        for (x, y) in pts:
            data += struct.pack('fff', float(x), float(y), 0.1)

        msg = PointCloud2()
        msg.header = header; msg.height = 1; msg.width = len(pts)
        msg.fields = fields; msg.is_bigendian = False
        msg.point_step = 12; msg.row_step = 12 * len(pts)
        msg.data = bytes(data); msg.is_dense = True
        self._pub.publish(msg)
```

**보완 2 — nav2_params.yaml robot_layer 추가**
...
local_costmap:
  local_costmap:
    ros__parameters:
      plugins: [obstacle_layer, elevation_layer, low_obs_layer, robot_layer, inflation_layer]

      robot_layer:
        plugin: "nav2_costmap_2d::ObstacleLayer"
        observation_sources: robot_poses
        robot_poses:
          topic: /swarm/robot_poses_array
          data_type: "PointCloud2"
          sensor_frame: map
          obstacle_max_range: 3.0   # 3m 이내 로봇만 반영
          raytrace_max_range: 3.0
          clearing: true            # 이동 시 이전 장애물 자동 삭제
          marking: true
...
| 메커니즘 | 동작 시점 | 커버 범위 |
|----------|----------|-----------|
| Nav2 Costmap scan (ObstacleLayer) | 이동 중 | 벽·잔해 회피 |
| Elevation Layer | 이동 중 | 낮은 장애물 회피 |
| **InterRobotCostmapLayer (B4 신규)** | **이동 중** | **로봇 ↔ 로봇 충돌 방지** |
| MMPF robot_repulsion | Frontier 선택 시 | 목표 중복 방지 |
| Rendezvous 프로토콜 | 통신 단절 시 | 집결 충돌 완화 |

**완료 조건**
- [ ] `inter_robot_costmap_layer.py` 실행 후 `/swarm/robot_poses_array` 5Hz 퍼블리시 확인
- [ ] 타 로봇 1m 이내 접근 시 Nav2 local costmap에 장애물 마킹 확인 (RViz2)
- [ ] 타 로봇 이동 후 0.3s 내 costmap 장애물 자동 삭제 확인
- [ ] 좁은 통로(1.0m 폭) 2대 교행 시 충돌 없이 우회 경로 생성 확인
- [ ] 5대 동시 실행 시 노드 CPU 사용률 < 5% 확인 (`top` 기준)
...
| v1.2 | 2026-03-17 | **[B4] Inter-Robot 동적 장애물 등록 반영 (M06)**: Nav2 local costmap이 이동 중 타 로봇을 동적 장애물로 인식하지 못하는 문제 보완. `inter_robot_costmap_layer.py` 신규 추가 — /robot_N/pose 5대 수집, TTL 0.3s 관리, PointCloud2 변환, /swarm/robot_poses_array 5Hz 퍼블리시. nav2_params.yaml local_costmap에 robot_layer(ObstacleLayer) 추가(obstacle_max_range 3m, clearing true). M06 완료 조건에 [B4] 항목 3개 추가. 섹션 12.4 신규 추가(역할 분담 표, 완료 조건 5개). |

---

*M01부터 순서대로 진행하되, 같은 Phase 내 병렬 구현 가능한 모듈은 동시 진행 권장.*  
*각 모듈 완료 후 단위 테스트 통과 확인 → 다음 Phase 진입 조건 충족 시 진행.*  
*보완 사항 [B1][B2][B3][B4]은 해당 모듈 구현 시 반드시 함께 적용.*
